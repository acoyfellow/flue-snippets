...
for i in $(seq 1 20); do
	RESPONSE=$(curl -sS -m 120 "$URL/workflows/ai-gateway?wait=result" \
		-H 'content-type: application/json' \
		-d '{"message":"Say one short word."}' || true)
	printf '%s' "$RESPONSE" | grep -q '"gateway"' && break
	[ "$i" = "20" ] && { echo "assert never returned a gateway answer: $RESPONSE" >&2; exit 1; }
	sleep 4
done
printf '%s\n' "$RESPONSE"
...
let s=""; process.stdin.on("data", d => s += d).on("end", () => {
	...
	if (!result || typeof result.answer !== "string" || result.answer.length === 0 || typeof result.gateway !== "string") {
		console.error("ai-gateway assertion failed", body);
		process.exit(1);
	}
	console.log("ai-gateway assertion passed:", JSON.stringify({answer: result.answer, gateway: result.gateway}));
});
'
