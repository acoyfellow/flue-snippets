...
for i in $(seq 1 20); do
	RESPONSE=$(curl -sS -m 120 "$URL/workflows/workers-ai?wait=result" \
		-H 'content-type: application/json' \
		-d '{"message":"Say one short word."}' || true)
	printf '%s' "$RESPONSE" | grep -q '"answer"' && break
	[ "$i" = "20" ] && { echo "assert never returned an answer: $RESPONSE" >&2; exit 1; }
	sleep 4
done
printf '%s\n' "$RESPONSE"
...
let s=""; process.stdin.on("data", d => s += d).on("end", () => {
	...
	if (!result || typeof result.answer !== "string" || result.answer.length === 0) {
		console.error("workers-ai assertion failed (empty/missing answer)", body);
		process.exit(1);
	}
	console.log("workers-ai assertion passed:", JSON.stringify(result.answer));
});
'
