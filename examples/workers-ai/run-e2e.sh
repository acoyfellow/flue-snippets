...
EXAMPLE="$ROOT/examples/workers-ai"
cd "$EXAMPLE"
...
CONV="e2e-workers-ai-$(date +%s)"
for i in $(seq 1 40); do
	code=$(curl -sS -o /dev/null -w '%{http_code}' -m 60 "$URL/agents/workers-ai/warmup-$CONV" \
...
for i in $(seq 1 20); do
	ADMIT=$(curl -sS -m 60 "$URL/agents/workers-ai/$CONV" \
	-H 'content-type: application/json' \
		-H "x-api-key: $SNIPPET_API_KEY" \
	-d '{"kind":"user","body":"Say one short word."}' || true)
...
for i in $(seq 1 60); do
	RESPONSE=$(curl -sS -m 60 "$URL/agents/workers-ai/$CONV" -H 'accept: application/json' -H "x-api-key: $SNIPPET_API_KEY" || true)
	printf '%s' "$RESPONSE" | grep -q '"role":"assistant"' && break
	[ "$i" = "60" ] && { echo "conversation never returned an assistant reply: $RESPONSE" >&2; exit 1; }
	sleep 3
done
...
let s=""; process.stdin.on("data", d => s += d).on("end", () => {
	...
	const answer = snapshot.messages
		...
		.join("");
	if (answer.length === 0) {
		console.error("workers-ai assertion failed (empty/missing answer)", snapshot);
		process.exit(1);
	}
	console.log("workers-ai assertion passed:", JSON.stringify(answer));
});
'
