#!/usr/bin/env bash
# run-e2e.sh, full E2E for braintrust-eval (Flue 1.0 workflow + Braintrust Eval).
set -euo pipefail
cd "$(dirname "$0")"
ROOT="$(cd ../.. && pwd)"
...
: "${BRAINTRUST_API_KEY:?BRAINTRUST_API_KEY must be set}"
...
for i in $(seq 1 20); do
  code=$(curl -sS -m 120 -o /tmp/warmup-body -w '%{http_code}' \
    "$WORKER_URL/workflows/braintrust-eval?wait=result" \
...
AGENT_URL="${WORKER_URL}/workflows/braintrust-eval?wait=result" \
  BRAINTRUST_API_KEY="$BRAINTRUST_API_KEY" \
  bun run gateproof.plan.ts
echo "::endgroup::"

echo "✅ recipe braintrust-eval E2E pass"
