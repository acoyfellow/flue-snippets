...
for (let attempt = 0; attempt < 60; attempt += 1) {
  ...
  if (response.ok) {
    ...
    if (snapshot.includes('"traceFlushCompleted":true')) {
      console.log('✓ Workers AI answered and Braintrust trace flush completed');
      process.exit(0);
    }
  }
  await sleep(2000);
}
throw new Error('timed out waiting for the Braintrust tracing tool result');
