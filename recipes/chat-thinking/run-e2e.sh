#!/usr/bin/env bash
# run-e2e.sh, full E2E for the chat-thinking recipe (Flue 1.0 workflow + Think DO).
set -euo pipefail
cd "$(dirname "$0")"
ROOT="$(cd ../.. && pwd)"
...
for i in $(seq 1 20); do
  code=$(curl -sS -m 120 -o /tmp/warmup-body -w '%{http_code}' \
    "$WORKER_URL/workflows/chat-thinking?wait=result" \
    -H 'content-type: application/json' \
    -d '{"chatId":"warmup","message":"warmup"}' 2>/dev/null || echo "000")
...
AGENT_URL_BASE="${WORKER_URL}/workflows/chat-thinking" \
  bun run gateproof.plan.ts
echo "::endgroup::"

echo "✅ recipe chat-thinking E2E pass"
