#!/usr/bin/env bash
#
# run-e2e.sh, full E2E for the dynamic-workflow recipe (Flue 1.0).
...
for i in $(seq 1 20); do
  code=$(curl -sS -m 120 -o /tmp/warmup-body -w '%{http_code}' \
    "$WORKER_URL/workflows/dynamic-workflow?wait=result" \
    -H 'content-type: application/json' \
    -d '{"runId":"warmup","action":"status"}' 2>/dev/null || echo "000")
...
AGENT_URL_BASE="${WORKER_URL}/workflows/dynamic-workflow" \
