#!/usr/bin/env bash
#
# run-e2e.sh, full E2E for the dynamic-workflow snippet.
#
# Deploys TWO Workers (runner + Flue agent), warms up the agent,
...
# alchemy.run.ts prints both URLs but the CLIENT (Flue agent) URL is
# printed LAST, so `tail -1` grabs the client (which is what we probe).
WORKER_URL=$(grep -oE 'https://[a-z0-9-]+\.[a-z0-9-]+\.workers\.dev' "$DEPLOY_LOG" | tail -1)
if [ -z "$WORKER_URL" ]; then
  echo "::error::failed to capture deployed URL"
  exit 1
fi
echo "client deployed: $WORKER_URL"
echo "::endgroup::"
...
for i in $(seq 1 12); do
  code=$(curl -sS -m 60 -o /tmp/warmup-body -w '%{http_code}' -X POST \
    "$WORKER_URL/agents/dynamic-workflow/warmup" \
...
AGENT_URL_BASE="${WORKER_URL}/agents/dynamic-workflow" \
  bun run gateproof.plan.ts
echo "::endgroup::"

echo "✅ snippet dynamic-workflow E2E pass"
