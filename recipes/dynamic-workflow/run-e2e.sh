...
echo "::endgroup::"
echo "::group::warmup"
WARMUP_URL="$WORKER_URL/agents/dynamic-workflow/warmup"
for i in $(seq 1 40); do
  code=$(curl -sS -m 120 -o /tmp/warmup-body -w '%{http_code}' "$WARMUP_URL" -X POST -H 'content-type: application/json' \
		-H "x-api-key: $SNIPPET_API_KEY" -d '{"kind":"user","body":"{\"runId\":\"warmup\",\"action\":\"status\"}"}' 2>/dev/null || echo "000")
  if [ "$code" = "202" ] || [ "$code" = "200" ]; then
    curl -sS -m 120 "$WARMUP_URL" >/tmp/warmup-body || true
...
AGENT_URL_BASE="${WORKER_URL}/agents/dynamic-workflow" bun run gateproof.plan.ts
echo "::endgroup::"
echo "✅ recipe dynamic-workflow E2E pass"
