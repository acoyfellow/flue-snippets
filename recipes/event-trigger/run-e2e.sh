...
npx wrangler deploy --config "$DEPLOY_CONFIG" --var "SNIPPET_API_KEY:$SNIPPET_API_KEY" --var "EVENT_HMAC_SECRET:$EVENT_HMAC_SECRET" 2>&1 | tee "$DEPLOY_LOG"
WORKER_URL=$(grep -Eo 'https://[A-Za-z0-9.-]+\.workers\.dev' "$DEPLOY_LOG" | tail -1)
test -n "$WORKER_URL"
echo "deployed: $WORKER_URL"
echo "::endgroup::"
echo "::group::warmup"
WARMUP_URL="$WORKER_URL/agents/event-trigger/warmup"
for i in $(seq 1 40); do
  code=$(curl -sS -m 60 -o /tmp/warmup-body -w '%{http_code}' "$WARMUP_URL" -X POST -H 'content-type: application/json' \
		-H "x-api-key: $SNIPPET_API_KEY" -d '{"kind":"user","body":"{\"source\":\"generic\",\"event\":{\"kind\":\"warmup\"}}"}' 2>/dev/null || echo "000")
  if [ "$code" = "202" ] || [ "$code" = "200" ]; then
    curl -sS -m 60 "$WARMUP_URL" >/tmp/warmup-body || true
    echo "  agent route live after $i attempts"
    break
  fi
  if [ "$i" = "40" ]; then echo "::error::agent route still failing (HTTP $code)"; exit 1; fi
  sleep 4
done
echo "::endgroup::"
echo "::group::gateproof plan against $WORKER_URL"
AGENT_URL_BASE="$WORKER_URL" EVENT_HMAC_SECRET="$EVENT_HMAC_SECRET" bun run gateproof.plan.ts
echo "::endgroup::"
