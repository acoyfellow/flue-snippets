#!/usr/bin/env bash
# run-e2e.sh, full E2E for the gateway-lab recipe (Flue 1.0 workflow + Lab).
set -euo pipefail
cd "$(dirname "$0")"
ROOT="$(cd ../.. && pwd)"
...
for i in $(seq 1 20); do
  code=$(curl -sS -m 120 -o /tmp/warmup-body -w '%{http_code}' \
    "$WORKER_URL/workflows/gateway-lab?wait=result" \
...
AGENT_URL="${WORKER_URL}/workflows/gateway-lab?wait=result" bun run gateproof.plan.ts
echo "::endgroup::"

echo "✅ recipe gateway-lab E2E pass"
