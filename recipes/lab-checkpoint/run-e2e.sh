#!/usr/bin/env bash
#
# run-e2e.sh, full E2E for the lab-checkpoint snippet.
...
for i in $(seq 1 12); do
  code=$(curl -sS -m 120 -o /tmp/warmup-body -w '%{http_code}' -X POST \
    "$WORKER_URL/agents/lab-checkpoint/warmup" \
...
AGENT_URL="${WORKER_URL}/agents/lab-checkpoint/gp-${STAGE}" \
LAB_URL="${LAB_URL}" \
  bun run gateproof.plan.ts
echo "::endgroup::"

echo "✅ snippet lab-checkpoint E2E pass"
