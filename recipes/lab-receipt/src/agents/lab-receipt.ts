...
type RuntimeEnv = { LAB_URL: string };
...
export function LabReceipt() {
  ...
  useTool(
    defineTool({
      name: 'prompt_with_receipt',
      description: 'Answer the delivered prompt and persist its input and output in a Lab receipt.',
      harness: true,
      async run({ harness }) {
        ...
        const result = await lab.createReceipt({
          source: 'flue-snippets/recipes/lab-receipt',
          action: 'prompt',
          input: { message },
          output: { answer },
  ...
  return 'Call prompt_with_receipt exactly once for every delivered JSON request. Return its output as JSON without changing field names.';
}
