...
npx wrangler deploy --config "$DEPLOY_CONFIG" --var "SNIPPET_API_KEY:$SNIPPET_API_KEY" --var "MCP_URL:$MCP_URL" 2>&1 | tee "$DEPLOY_LOG"
WORKER_URL=$(grep -Eo 'https://[A-Za-z0-9.-]+\.workers\.dev' "$DEPLOY_LOG" | tail -1)
test -n "$WORKER_URL"
echo "deployed: $WORKER_URL (MCP_URL=$MCP_URL)"
echo "::endgroup::"
echo "::group::warmup"
WARMUP_URL="$WORKER_URL/agents/mcp-client/warmup"
for i in $(seq 1 40); do
  code=$(curl -sS -m 90 -o /tmp/warmup-body -w '%{http_code}' "$WARMUP_URL" -X POST -H 'content-type: application/json' \
		-H "x-api-key: $SNIPPET_API_KEY" -d '{"kind":"user","body":"{\"text\":\"warmup\"}"}' 2>/dev/null || echo "000")
  if [ "$code" = "202" ] || [ "$code" = "200" ]; then curl -sS -m 90 "$WARMUP_URL" >/tmp/warmup-body || true; echo "  agent route live after $i attempts"; break; fi
...
AGENT_URL_BASE="${WORKER_URL}/agents/mcp-client" bun run gateproof.plan.ts
echo "::endgroup::"
