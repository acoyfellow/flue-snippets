...
# alchemy.run.ts prints both URLs but the CLIENT url is printed LAST,
# so `tail -1` grabs the client (which is what we probe).
...
for i in $(seq 1 12); do
  code=$(curl -sS -m 120 -o /tmp/warmup-body -w '%{http_code}' -X POST \
    "$WORKER_URL/agents/mcp-client/warmup" \
    -H 'content-type: application/json' \
    -d '{"text":"warmup"}' 2>/dev/null || echo "000")
...
AGENT_URL_BASE="${WORKER_URL}/agents/mcp-client" \
  bun run gateproof.plan.ts
echo "::endgroup::"

echo "✅ snippet mcp-client E2E pass"
