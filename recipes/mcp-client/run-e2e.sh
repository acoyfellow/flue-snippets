#!/usr/bin/env bash
# run-e2e.sh, full E2E for the mcp-client recipe (Flue 1.0 workflow + co-hosted MCP server).
set -euo pipefail
cd "$(dirname "$0")"
ROOT="$(cd ../.. && pwd)"
...
# The MCP server is co-hosted on this same Worker, so MCP_URL is the Worker's
# own /mcp endpoint. The workers.dev URL is deterministic from the name, so we
# can set it before the first deploy.
MCP_URL="https://${WORKER_NAME}.coy.workers.dev/mcp"
npx wrangler deploy --config "$DEPLOY_CONFIG" --var "MCP_URL:$MCP_URL" 2>&1 | tee "$DEPLOY_LOG"
WORKER_URL=$(grep -Eo 'https://[A-Za-z0-9.-]+\.workers\.dev' "$DEPLOY_LOG" | tail -1)
test -n "$WORKER_URL"
echo "deployed: $WORKER_URL (MCP_URL=$MCP_URL)"
echo "::endgroup::"

echo "::group::warmup"
for i in $(seq 1 20); do
  code=$(curl -sS -m 90 -o /tmp/warmup-body -w '%{http_code}' \
    "$WORKER_URL/workflows/mcp-client?wait=result" \
    -H 'content-type: application/json' -d '{"text":"warmup"}' 2>/dev/null || echo "000")
...
AGENT_URL_BASE="${WORKER_URL}/workflows/mcp-client" bun run gateproof.plan.ts
echo "::endgroup::"

echo "✅ recipe mcp-client E2E pass"
