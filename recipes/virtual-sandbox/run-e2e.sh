#!/usr/bin/env bash
#
# run-e2e.sh, full E2E for the virtual-sandbox snippet.
...
for i in $(seq 1 12); do
  code=$(curl -sS -m 120 -o /tmp/warmup-body -w '%{http_code}' -X POST \
    "$WORKER_URL/agents/virtual-sandbox/warmup" \
...
AGENT_URL_BASE="${WORKER_URL}/agents/virtual-sandbox" \
  bun run gateproof.plan.ts
echo "::endgroup::"

echo "✅ snippet virtual-sandbox E2E pass"
